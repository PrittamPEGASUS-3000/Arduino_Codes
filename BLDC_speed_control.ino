...
#include<Servo.h>
Servo esc5; //objct esc of class servo
SoftwareSerial bt(0,1);//RX,TX objct bt of class softwareserial
void setup()
{
  //motor code
  esc5.attach(5,900,2000);
  esc5.writeMicroseconds(900);//initialize signal to 900
  delay(1000);
  //bluetooth
  bt.begin(9600);
  Serial.begin(9600);
...
void loop()
{
  int val;//bluetooth input
  if(bt.available())
  {
    val=bt.read();//data received to be put in val 
    //val value is 0 to 1023
    val=map(val, 48, 60, 1000, 2000);//val mapping from min to max
    esc5.writeMicroseconds(val);
    delay(2000);
  }
}*/

//eta 2nd prog jani na konta kaj korbe :)
#include <Servo.h>
